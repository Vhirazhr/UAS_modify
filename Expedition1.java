...
public class Expedition1 {
    ...
    // untuh penambahan kayu
    static boolean useWoodProtection = false;
    ...
    static void safety() {
        System.out.print("Do you want additional protection using wood? (yes/no): ");
        String protectionChoice = scanner.nextLine();

        if (protectionChoice.equals("yes")) {

            useWoodProtection = true;
            int woodProtectionCost = weight * 5000;
            distanceCost += woodProtectionCost;
            System.out.println("Wood protection added. Additional cost: Rp. " + woodProtectionCost);
        } else {

            useWoodProtection = false;
            System.out.println("No additional protection selected.");
            ClearScreen();
            MainMenuUser();
        }
    }
